...
import type { StackItem, StackActionResult, StackProgressEvent, ServerCapabilities } from "../domain/stack.types.js";
...
export const stackWailsApi = {
  ...
  async deployStack(profileId: string, stackId: string): Promise<StackActionResult> {
    return await (StackService as any).DeployStack(profileId, stackId);
  },

  async stopStack(profileId: string, stackId: string): Promise<StackActionResult> {
    return await (StackService as any).StopStack(profileId, stackId);
  },

  async removeStackRemote(profileId: string, stackId: string, deleteVolumes: boolean = false): Promise<StackActionResult> {
    return await (StackService as any).RemoveStackRemote(profileId, stackId, deleteVolumes);
  },

  async getLogs(profileId: string, stackId: string, tail: number = 200): Promise<string> {
    return await (StackService as any).GetStackLogs(profileId, stackId, tail);
  },

  async getServerCapabilities(profileId: string = "default"): Promise<ServerCapabilities> {
  ...
  subscribeToDeployProgress(callback: (event: StackProgressEvent) => void): () => void {
    return Events.On("stacks:deploy:progress", (ev) => callback(ev.data as StackProgressEvent));
  },

  subscribeToDeployComplete(callback: (event: StackProgressEvent) => void): () => void {
    return Events.On("stacks:deploy:complete", (ev) => callback(ev.data as StackProgressEvent));
  },

  subscribeToDeployFailed(callback: (event: StackProgressEvent) => void): () => void {
    return Events.On("stacks:deploy:failed", (ev) => callback(ev.data as StackProgressEvent));
  },
};
