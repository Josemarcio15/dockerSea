<script lang="ts">
  ...
  function applyJson() {
    try {
      ...
      volumes = (Array.isArray(cfg.volumes) ? cfg.volumes : []).map(
        (v: any) => ({ host: v.host ?? "", container: v.container ?? "" }),
      );
      commands = Array.isArray(cfg.commands)
        ? cfg.commands.map((c: any) => String(c ?? ""))
        : [""];
      const matchingProfile = savedConfigs.find(
        (saved) => saved.name === profileName,
      );
  ...
  async function importJson(event: Event) {
    ...
    try {
      ...
      loadedProfileName = "";
      ports = (
        Array.isArray(cfg.ports) ? cfg.ports : emptyConfigTemplate.ports
      ).map((p: any) => ({
        external: p.port ?? p.external ?? "",
        internal: p["port-intern"] ?? p.internal ?? "",
      }));
      envs = (
        Array.isArray(cfg.envs) ? cfg.envs : emptyConfigTemplate.envs
      ).map((e: any) => ({
        name: e.name ?? "",
        value: e.value ?? "",
      }));
      volumes = (
        Array.isArray(cfg.volumes) ? cfg.volumes : emptyConfigTemplate.volumes
      ).map((v: any) => ({
        host: v.host ?? "",
      ...
      >
        ...
        <div class="lg:col-start-2 flex flex-col min-w-0 gap-4">
          ...
          >
            <button
              type="button"
              class="flex-1 rounded-lg px-3 py-2 text-xs font-bold {editorTab ===
              'fields'
                ? 'bg-white dark:bg-slate-800 text-violet-600 shadow-sm'
                : 'text-slate-500'}"
              onclick={() => (editorTab = "fields")}>Campos</button
            >
            <button
              type="button"
              class="flex-1 rounded-lg px-3 py-2 text-xs font-bold {editorTab ===
          ...
          {#if editorTab === "json"}
            ...
            >
              <div class="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p class="text-[10px] text-slate-500">
                    Selecione um perfil acima, depois edite ou cole as variáveis
                    aqui.
                  </p>
                </div>
              ...
              />
              {#if jsonMessage}<p
                  class="text-[10px] font-semibold {jsonMessage.includes(
                    'inválido',
                  )
                    ? 'text-red-500'
                    : 'text-emerald-600'}"
          ...
          {#if editorTab === "fields"}
            ...
            <div class="flex flex-col gap-1">
              ...
              >
                ...
                <option value="always"
                  >{t("containers.card_restart_always")}</option
                >
