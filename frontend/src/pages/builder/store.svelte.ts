...
export const builderStore: BuilderStore = {
  ...
  set customTag(value: string) {
    customTag = value;
  },
  get folderName() {
    return folderNameFromPath(currentPath);
  },
  get defaultTag() {
    return currentPath ? tagFromPath(currentPath) : "";
  },
  get effectiveTag() {
    return customTag.trim() ? sanitizeTag(customTag) : this.defaultTag;
  },
  get canBuild() {
    return this.effectiveTag.length > 0 && hasDockerfile && status === "ready";
  },

  async loadSavedPaths() {
    try {
      savedPaths = ((await api.listSavedPaths()) || []).map(
        (item) => item.path,
      );
    } catch {
      /* optional data */
