...
import type { VpsServer, ImageHistoryItem } from "./api";
...
export function createImagesStore(
  ...
  async function fetchHistory() {
    try {
      ...
      const hist = await pageApi.listHistory(profileId);
      imageHistory = hist || [];
    } catch (e) {
      imageHistory = [];
    }
  ...
  async function fetchSavedConfigs() {
    try {
      savedConfigs = (await pageApi.listContainerConfigs(getProfileId())) || [];
    } catch {
      savedConfigs = [];
    }
  }
  ...
  async function saveProfile(profile: Record<string, unknown>) {
    await pageApi.saveContainerConfig({
      ...profile,
  ...
  async function handlePull(imageName: string) {
    ...
    const profileId = getProfileId();
    pageApi
      .pull(server, imageName, profileId)
      .then((res) => {
        if (!res.success) {
  ...
  async function handleDeleteSelected() {
    ...
    try {
      const result = await pageApi.remove(server, selectedImageIds);
      if (result.success) {
        notifySuccess(
  ...
  async function handlePruneUnused() {
    ...
    notifySuccess(t("images.delete_progress", { count: unusedIds.length }));

    try {
      const result = await pageApi.remove(server, unusedIds);
      if (result.success) {
        notifySuccess(
          t("images.delete_success", {
  ...
  function toggleAllTransfer() {
    if (selectedTransferIds.length === sourceImages.length) {
      ...
      selectedTransferIds = sourceImages.map((img) => getTransferImageKey(img));
    }
  }
  ...
  async function handleTransfer() {
    ...
    try {
      const result = await pageApi.transferImages(
        srcServer,
        dstServer,
        selectedTransferIds,
      );
      ...
      notifyError(
        t("images.transfer_error").replace("{error}", e.message || String(e)),
      );
    } finally {
      clearInterval(timer);
      unsubProgress();
